...
def train_val():
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope('GRAPH', reuse=None):
            [train_x, train_y_exp, train_y_pse, train_loss, train_acc_exp, train_acc_pse, train_op, train_summary, train_crop_x] = build_graph(True)
        with tf.variable_scope('GRAPH', reuse=True):
            [val_x, val_y_exp, val_y_pse, val_loss, val_acc_exp, val_acc_pse, _, val_summary, val_crop_x] = build_graph(False)
        ...
        with tf.Session() as sess:
            ...
            for epoch in xrange(SNAPSHOT_RESUME_FROM+1, EPOCH_MAX+1):
                ...
                for itr in xrange(ITER_COUNT):
                    ...
                    [_, train_loss_val, train_acc_exp_val, train_acc_pse_val, train_summary_val, train_crop_x_val] =\
                        sess.run([train_op, train_loss, train_acc_exp, train_acc_pse, train_summary, train_crop_x], feed_dict={train_x: train_x_val, train_y_exp: train_y_exp_val, train_y_pse: train_y_pse_val})
                ...
                for itr in xrange(ITER_COUNT):
                    ...
                    [val_loss_val, val_acc_exp_val, val_acc_pse_val, val_summary_val, val_crop_x_val] =\
                        sess.run([val_loss, val_acc_exp, val_acc_pse, val_summary, val_crop_x], feed_dict={val_x: val_x_val, val_y_exp: val_y_exp_val, val_y_pse: val_y_pse_val})
