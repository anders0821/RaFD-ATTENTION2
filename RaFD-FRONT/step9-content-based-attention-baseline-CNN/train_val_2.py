...
def build_graph(train):
    ...
    with tf.name_scope('FLAT'):
        mlp = tf.reshape(mlp, [-1, 7*9*c4], name='flat')
    ...
    with tf.name_scope('FC'):
        mlp = tf.nn.xw_plus_b(mlp, weight([7*9*c4, 7], name='w7'), zero_bias([7], name='b7'), name='fc')
    ...
    return [x, y, loss, prob, acc, train_op, summary, crop_x]
...
def train_val():
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope('GRAPH', reuse=None):
            [train_x, train_y, train_loss, _, train_acc, train_op, train_summary, train_crop_x] = build_graph(True)
        with tf.variable_scope('GRAPH', reuse=True):
            [val_x, val_y, val_loss, _, val_acc, _, val_summary, val_crop_x] = build_graph(False)
        ...
        with tf.Session() as sess:
            ...
            for epoch in xrange(SNAPSHOT_RESUME_FROM+1, EPOCH_MAX+1):
                ...
                for itr in xrange(ITER_COUNT):
                    ...
                    [_, train_loss_val, train_acc_val, train_summary_val, train_crop_x_val] = sess.run([train_op, train_loss, train_acc, train_summary, train_crop_x], feed_dict={train_x: train_x_val, train_y: train_y_val})
                ...
                for itr in xrange(ITER_COUNT):
                    ...
                    [val_loss_val, val_acc_val, val_summary_val, val_crop_x_val] = sess.run([val_loss, val_acc, val_summary, val_crop_x], feed_dict={val_x: val_x_val, val_y: val_y_val})
                ...
                # save X-Y-MASK.mat
                with h5py.File(OUTPUT_DIR+'X-Y-MASK.mat', 'w') as h5:
